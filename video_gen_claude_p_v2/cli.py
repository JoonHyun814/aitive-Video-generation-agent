...
python -m video_gen_claude_p_v2.cli --scenario-json <scenario.json 경로> [--plan-only]
python -m video_gen_claude_p_v2.cli --scenario-json <scenario.json 경로> --from-plan <plan.json 경로>

Pipeline:
  1. agent_loop.run_plan_generation()으로 계획 수립 에이전트를 실행한다 -- 매뉴얼
     tool-calling 루프(web_fetch/web_search)로 반복 조사한 뒤 GenerationPlan을 구조화
     출력한다. 검증 실패 시 최대 --repair-attempts번 repair 호출(같은 연구 메시지 기반).
     모든 모델/도구 호출은 trace.json/trace.html로 남는다.
     (--from-plan이 지정되면 이 단계 전체를 건너뛰고 기존 plan.json을 그대로 쓴다.)
  2. (--plan-only가 아니면) 결정론적 Python 실행기가 ComfyUI Workflow API를 호출해
     entity 레퍼런스 이미지 -> 컷 프레임 -> 브랜드 앵커 프레임 -> 최종 영상을 순서대로
     만든다.
"""
...
from .schema import GenerationPlan, lean_json_schema
...
def _format_plan_report(validation: ValidationResult, cost_usd, repair_attempts_used: int) -> str:
    ...
    lines.append(f"- 누적 비용(USD, 계획 수립 에이전트 호출만): {cost_usd}")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="시나리오 -> 이미지/영상 생성 파이프라인 (에이전트 + ComfyUI)")
    ap.add_argument("--scenario-json", required=True, help="scenario_agent_claude(_litellm) 산출물 scenario.json 경로")
    ap.add_argument("--output-dir", default=str(Path(PROJECT_DIR) / "output" / "video_gen_v2"))
    ap.add_argument("--model", default=None, help="기본값은 .env의 LITELLM_MODEL")
    ...
    ap.add_argument(
        "--from-plan",
        help="이미 생성된 plan.json 경로. 지정하면 계획 수립 에이전트 호출(및 repair)을 "
    ...
    if args.from_plan:
        ...
        if not from_plan_path.is_file():
            ...
            return 1
        print(f"[1/4] 기존 계획 재사용 (계획 수립 에이전트 생략): {from_plan_path}")
        ...
        print(f"[1/4] 계획 수립 에이전트 실행 중 (model={args.model or '기본값'}, budget=${args.max_budget_usd})...")
        result = agent_loop.run_plan_generation(
            system_prompt=SYSTEM_PROMPT, task_prompt=task_prompt, schema=schema,
        ...
        if structured_output is None and not messages:
        ...
        while (
            ...
            repair_result = agent_loop.run_repair(
                messages=messages, errors=validation.errors, schema=schema,
                previous_output=structured_output, model=args.model,
                attempt=repair_attempts_used,
            )
            trace += repair_result.trace
            if repair_result.total_cost_usd is None:
                cost_known = False
            elif cost_known:
                total_cost_usd += repair_result.total_cost_usd
            if not repair_result.ok:
                print(f"repair 호출 실패: {repair_result.error_message}", file=sys.stderr)
                break
            structured_output = repair_result.structured_output
            plan, validation = _parse_and_validate(structured_output, scenario)

        cost_usd = total_cost_usd if cost_known else None
    ...
    if trace:
        ...
        (out_dir / "trace.html").write_text(
            render_trace_html(trace, {"product_name": product_name, "total_cost_usd": cost_usd if isinstance(cost_usd, float) else None}),
            encoding="utf-8",
        )
    if messages:
    ...
    print("[3/4] ComfyUI 파이프라인 실행 중 (entity 이미지 -> 컷 프레임 -> 앵커 프레임 -> 최종 영상)...")
    ...
    trace_html_rel = "trace.html" if (out_dir / "trace.html").exists() else None
    (out_dir / "report.html").write_text(render_html(manifest, plan, scenario, trace_html_rel), encoding="utf-8")
    ...
    print(f"\n[4/4] 출력 위치: {out_dir}")
    print(f"이미지 단계: {n_ok}/{n_total} 성공, 앵커 프레임: {manifest.anchor_frame.status}, 최종 영상: {final_status}")
