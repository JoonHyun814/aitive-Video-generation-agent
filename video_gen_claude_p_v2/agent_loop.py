"""Manual tool-calling agent loop for the planning stage -- adapted from
scenario_agent_claude_litellm/agent_loop.py for GenerationPlan instead of
Scenario. See this package's README for why planning is a real multi-turn
tool-calling agent (trace-recorded) while the actual Flux/Qwen-Edit/MiniMax
generation calls happen afterward in executor.py, not as agent tool calls.

Every model/tool call is appended to a flat `trace` list (usage, cache
fields, tool name/arguments, result previews) so the full planning run can be
written to disk and rendered as a step-by-step HTML log -- see
trace_render.py.
"""
...
def _final_json_schema(schema: dict) -> dict:
    return {"type": "json_schema", "json_schema": {"name": "generation_plan", "schema": schema, "strict": True}}
...
def run_plan_generation(
    *,
    ...
    structured, err, final_usage_cost, _raw = _final_structured_call(
