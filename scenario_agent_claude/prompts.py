"""Prompt construction for the ad-scenario agent.

The ad-effectiveness framework summaries are inlined here (rather than left
as `Read docs/ad/*.md` instructions) so they are guaranteed to be in context
every run regardless of whether the model chooses to read the files.
"""
...
RAG_USAGE_RULES = """\
## RAG 검색 도구 사용 규칙 (`chromadb-explorer-network` MCP 서버)

- `search_chromadb`: 자연어 의미 유사도 검색. 일반적인 컨셉/전략 탐색에 사용.
- `search_chromadb_hybrid`: 브랜드명·숫자·고유명사처럼 정확히 일치해야 하는 키워드가 쿼리에
  있을 때 `search_chromadb` 대신 사용 (dense + BM25 결합).
- `fetch_by_video_id`: 검색으로 이미 찾은 특정 video_id의 원본 레코드 전체를 가져올 때만 사용.
  새 광고를 찾는 용도가 아님.
- `search_visual`: 색감/구도/소품/조명 등 순수 시각적 특징으로 컷을 찾을 때. **반환되는
  `image_path`는 이 서버가 돌아가는 호스트의 파일시스템 경로다 — 실제로 열려고 시도하지 말고
  텍스트 힌트(어떤 비주얼 스타일이 이 카테고리에서 흔한지)로만 참고할 것.**
- `search_graph_pattern`: 개별 광고 검색이 아니라 여러 캠페인에 걸쳐 특정 서사 역할(role)에서
  자주 쓰인 크리에이티브 요소를 집계한 통계다. 씬의 `narrative_role`을 정한 뒤, 그 role로
  이 도구를 호출해 "이 역할에서 실제로 자주 쓰이는 연출 요소"를 참고하는 용도로 쓸 것.
  **`persona_category` 인자는 절대 넘기지 말 것** — 현재 데이터에 값이 채워져 있지 않아 항상
  빈 결과만 반환한다(알려진 서버 제약).
- 컬렉션: `category_analysis`(산업/타겟/USP), `scenario_analysis`(컨셉/내러티브/키메시지),
  ...
  (연출/크리에이티브 레퍼런스), `ad_visual_reference`(키프레임 이미지, `search_visual` 전용).
...
JS 렌더링 SPA 등)에도 **WebFetch를 이미지 URL에 직접 호출하면 그 이미지를 시각적으로 분석한
결과(이미지 속 텍스트 판독 + 색상/형태 등 외형 묘사)를 받을 수 있다.** 별도의 다운로드나
다른 도구가 필요 없다 — 이미지 URL을 인자로 WebFetch를 호출하기만 하면 된다.

1. 먼저 상세페이지 URL 자체를 WebFetch로 읽어라. 텍스트가 거의 없다면, 응답에 포함된 이미지
   URL들(상품 상세 이미지, 성분표/스펙 이미지 등 — 배너·아이콘·로고류는 제외) 중 제품 정보를
   담고 있을 만한 것을 골라 그 이미지 URL들에 대해 각각 WebFetch를 다시 호출하라.
2. 상세페이지 URL 자체가 이미지 파일이면(예: 응답이 이미지로 처리됨) 그 결과를 바로 사용하라.
3. 이미지 분석에서 얻은 정보도 반드시 SourcedClaim으로 기록한다:
   ...
   - 색상·형태·재질감 등 제품 외형은 `appearance`에 추가한다. 이 경우 `source_quote`는 원문
     ...
     남색 계열이며 상단에 노란색 띠 라벨이 있음"). 실제로 그 이미지를 WebFetch로 확인한
...
1. 제품 상세페이지 URL을 WebFetch로 읽어라. 텍스트를 추출할 수 없으면(이미지로만 구성된
   상세페이지, JS 렌더링 필요 등) 아래 "이미지 분석" 규칙에 따라 이미지 자체를 시각적으로
...
2. 상세페이지(텍스트+이미지)에 없는 정보가 필요하면 WebSearch로 브랜드 공식 정보를 보완
   조사할 수 있다(추측 금지).
...
def build_system_prompt(rag_server_name: str) -> str:
    return f"""\
...
{RAG_USAGE_RULES.replace("chromadb-explorer-network", rag_server_name)}
...
## 작업 방식
- 이용 가능한 도구(WebFetch, WebSearch, RAG MCP 도구)를 자유롭게, 그러나 목적에 맞게 사용하라.
- 최종 출력은 주어진 JSON 스키마를 그대로 따르는 구조화된 결과 하나여야 한다. 스키마 외의
  설명 텍스트를 덧붙이지 마라 — 구조화 출력 메커니즘이 최종 응답을 강제한다.
...
절차:
1. WebFetch로 상세페이지를 읽고 성분/기능/색상·외형 등 객관적 사실을 수집하라(사실 기반 원칙
   준수). 텍스트가 부족하면 이미지 분석 규칙에 따라 상세페이지의 제품 이미지에 WebFetch를
   직접 호출해 텍스트 판독과 외형(색상/형태) 분석을 수행하라. 필요하면 WebSearch로 보완하라.
