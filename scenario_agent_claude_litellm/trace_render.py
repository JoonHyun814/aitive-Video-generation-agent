...
token/cache/cost per step) into a single readable HTML log."""
...
<title>{_e(meta.get("product_name", "실행"))} — 실행 로그</title>
<style>
...
<div class="wrap">
  <header>
    <h1>{_e(meta.get("product_name", ""))} — 실행 로그</h1>
  ...
  {steps_html}
  <footer>scenario_agent_claude_litellm 실행 로그 — agent_loop.py의 trace 기록을 렌더링</footer>
</div>
</body>
</html>
"""
